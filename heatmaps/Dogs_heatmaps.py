...
###### Function: Generate heatmaps of Stanford Dogs ######
...
def parse_args():
    ...
    parser.add_argument('--root_path', type=str, default='data/Stanfors_Dogs',help='The path of dataset.')
...
mat_path = os.path.join(root_path, 'train_list.mat')
pic_root_path = os.path.join(root_path, 'images')
heatmaps_path = os.path.join(root_path, 'heatmaps')

split = scipy.io.loadmat(mat_path)['annotation_list']
...
def vit_base_patch16_384():
    model = VisionTransformer(img_size=384, patch_size=16, embed_dim=768,
                              depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, num_classes=120)
